...
exports.handler = async(event) => {
    if (!!!event.body || !!!event.body.deviceid) {
    ...
    const userDevice = await dynamo.getItem({
        ...
        ProjectionExpression: 'createdAt, #T',
        ExpressionAttributeNames: { '#T': 'type' }
    }).promise()
    if (!!!userDevice.Item) {
        return argumentErrorResponse('Device not pair')
    }
    ...
    if (!!!findDevice.Item) {
        return argumentErrorResponse('Device not found')
    }
    ...
    responseData.pairdate = userDevice.Item.createdAt.N
    responseData.isShare = userDevice.Item.type.S === 'share'
